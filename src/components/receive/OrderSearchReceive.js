...
const OrderSearchReceive = ({ orders, onOrderSelected, loading }) => {
  const [searchTerm, setSearchTerm] = useState("");
  const [searchType, setSearchType] = useState("all");
  const [statusFilter, setStatusFilter] = useState("in_workshop");
        ...
        <CardContent className="p-4">
          <div className="space-y-4">
            <div className="flex gap-2">
              ...
              {(searchTerm || statusFilter !== "in_workshop") && (
                <Button variant="outline" onClick={() => {setSearchTerm(""); setStatusFilter("in_workshop");}}>
                ...
                <CardContent className="p-4">
                  <div className="flex items-center justify-between">
                    <div className="flex-1 space-y-2">
                      ...
                      {order.workshop_delivery && (
                        <div className="text-xs text-blue-600 bg-blue-50 px-2 py-1 rounded">
                          📦 Workshop handling delivery
                        </div>
                      )}
                    </div>
                    ...
                    >
                      {order.status === 'completed' ? 'Completed' : 'Receive from Workshop'}
                    </Button>
                  </div>
                </CardContent>
              </Card>
...
export default OrderSearchReceive;